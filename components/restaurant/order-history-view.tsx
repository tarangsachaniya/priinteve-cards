 ...
 *
 * Every filter here is a server round trip, and previously each control called
 * router.push() on its own — so nothing on the page knew a query was in flight
 * and a click on a slow filter looked like a dead button. Routing through one
 * useTransition fixes that at the source: in the App Router the transition
 * stays pending until the *server* has re-rendered, so the spinner below
 * covers the actual query rather than just the client round trip.
 */
export function OrderHistoryView({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <OrderHistoryFilters filters={filters} menuItems={menuItems} onNavigate={navigate} />
