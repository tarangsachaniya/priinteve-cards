      ...
      {items.map((item, i) => {
        ...
        return (
          <a
            ...
            className={cn(
              "flex w-full items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-medium transition-transform hover:scale-[1.02]",
              item.style === "primary" && "text-white shadow-sm",
              item.style === "secondary" && "bg-muted text-foreground",
              item.style === "outline" && "border border-border bg-transparent text-foreground"
            )}
