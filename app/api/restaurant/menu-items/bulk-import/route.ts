...
export async function POST(req: Request) {
  ...
  try {
    // A real menu is a few hundred sequential writes — one per dish, plus a
    // couple of lookups per category — and Prisma's default interactive
    // transaction window is 5 seconds. That's enough for a handful of items
    // and nowhere near enough for a restaurant's full menu pasted in one go,
    // so both bounds are raised well past what even a large document needs.
    await db.$transaction(
      async (tx) => {
        for (const incomingCategory of incomingCategories) {
          ...
          // reloaded per category (not once up front) because a category
          // created earlier in this same import has items the loop itself
          // just added, which still need to be checked against.
          ...
          for (const incomingItem of incomingCategory.items) {
            ...
            // The item already exists. Its name, price and description are
            // left alone — this path only ever adds an option that isn't
            // there yet, it never edits what already is.
            const existingVariantNames = new Set(
              existingItem.variants.map((v) => v.name.toLowerCase())
            );
            const existingAddOnNames = new Set(
              existingItem.addOns.map((a) => a.name.toLowerCase())
            );
    ...
    // A transaction this size failing partway through must not look like it
    // silently created "some" of the menu — Prisma rolls the whole thing
    // back, so the honest answer is that nothing changed and the owner needs
    // a reason, not a generic 500.
    console.error("bulk menu import failed", err);
    const timedOut = err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2028";
    return NextResponse.json(
      {
        error: timedOut
          ? "This document took too long to import — try splitting it into smaller pastes (e.g. one per category)."
          : "Import failed partway through, so nothing was saved. Please try again.",
      },
