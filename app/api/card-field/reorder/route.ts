...
import { cardFieldReorderSchema } from "@/lib/validations/onboarding";
...
export async function POST(req: Request) {
  ...
  const parsed = cardFieldReorderSchema.safeParse(await req.json());
  if (!parsed.success) {
  ...
  const ownedFields = await db.cardField.findMany({
    where: { userId },
    select: { id: true },
  });
